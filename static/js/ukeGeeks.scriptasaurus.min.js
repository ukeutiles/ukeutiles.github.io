...
ukeGeeks.toolsLite=function(){var a={},g=/\s{2,}/g,d=/^\s+|\s+$/g;a.addClass=function(b,f){a.hasClass(b,f)||(b.className+=" "+f)};a.hasClass=function(b,a){return b.className.match(new RegExp("(\\s|^)"+a+"(\\s|$)"))};a.removeClass=function(b,d){a.hasClass(b,d)&&(b.className=b.className.replace(new RegExp("(\\s|^)"+d+"(\\s|$)")," "))};a.setClass=function(b,d,e){e?a.addClass(b,d):a.removeClass(b,d)};a.trim=function(b){return b.replace(d,"")};a.pack=function(b){return b.replace(g," ").replace(d,"")};
a.getElementsByClass=function(b,a,d){var c;a||(a=document);if(a.getElementsByClassName)return a.getElementsByClassName(b);var n=[];d||(d="*");a=a.getElementsByTagName(d);d=a.length;var g=new RegExp("(^|\\s)"+b+"(\\s|$)");for(c=b=0;b<d;b++)g.test(a[b].className)&&(n[c]=a[b],c++);return n};return a}();
...
a.match(d))&&1<c.length){c=c[1];for(var e=[],m=a.match(b);m&&1<m.length;)e.push(m[1]),a=a.replace(m[1],""),m=a.match(b);return new g(c,e)}return null},h=function(b,a){var d=[],m=[];var g=b.match(e);if(g)for(var g=4==g[1].length?g[1].match(p):g[1].match(k),h=0;h<g.length;h++){var l="x"==g[h]||"X"==g[h];d[h]=l?0:parseInt(g[h],10);m[h]=l}g=(g=b.match(f))?g[1]:null;(h=b.match(c))?(h=h[1],4==h.length&&(h=h.replace(p,"$1 ")),h=h.split(" ")):h=[];if(null===g||"frets"==g)return t.push('bad "define" instruction: chord name not found: '+
b),null;if(null===d)return t.push('bad "define" instruction: frets not found: '+b),null;for(var g=new ukeGeeks.data.expandedChord(g),l=h,h=[],r=ukeGeeks.settings.tuning,B=0;B<r.length;B++){var C=parseInt(d[B],10);0<C&&h.push(new ukeGeeks.data.dot(B,C,l.length-1>=B?parseInt(l[B],10):0))}if(a&&!(1>a.length))for(var D in a)(d=a[D].match(n))&&2<d.length&&h.push(new ukeGeeks.data.dot(parseInt(d[1],10)-1,parseInt(d[2],10),parseInt(d[3],10)));g.dots=h;g.muted=m;return g},t=[];a.runLine=function(c){return(c=
...
ukeGeeks.cpmParser=function(){var a={},g=1,d=!1,b="";a.init=function(){};a.parse=function(a){var k=new ukeGeeks.data.song;a=a.replace(/<\/?pre>/img,"").replace(/\x3c!--(.|\n)*?--\x3e/gm,"");var p;a=a.split("\n");var r=[],h=null,t,q;for(q in a)0<a[q].length&&"#"==a[q][0]||((t=f.blocks.test(a[q]))||null===h?(null!==h&&r.push(h),h=a[q],h={type:f.chorusBlock.test(h)?c.ChorusBlock:f.tabBlock.test(h)?c.TabBlock:c.TextBlock,lines:[]},t||h.lines.push(a[q])):(t=ukeGeeks.toolsLite.trim(a[q]),0<t.length&&h.lines.push(t)));
0<h.lines.length&&r.push(h);q=r;a=/\{[^}]+?:.*?\}/im;var r=/\{.+?:(.*)\}/gi,h=/\{(.+?)\s*:.*\}/gi,u;for(u in q)for(var w in q[u].lines)if(a.test(q[u].lines[w])){t=q[u].lines[w].replace(r,"$1");var z=q[u].lines[w].replace(h,"$1").toLowerCase(),z=z.replace(/\r/,""),y={type:"",lines:[]};switch(z){case "title":case "t":y.type=c.Title;break;case "artist":y.type=c.Artist;break;case "subtitle":case "st":y.type=c.Subtitle;break;case "album":y.type=c.Album;break;case "comment":case "c":y.type=c.Comment;break;
case "key":case "k":y.type=c.Key;break;case "define":y.type=c.ChordDefinition;break;case "ukegeeks-meta":y.type=c.UkeGeeksMeta;break;default:y.type="Undefined-"+z}y.lines[0]=ukeGeeks.toolsLite.trim(t);q[u].lines[w]=y}u=q;w=/\s*{\s*(column_break|colb|np|new_page)\s*}\s*/im;for(var x in u)for(var A in u[x].lines)if(w.test(u[x].lines[A]))switch(u[x].lines[A].replace(w,"$1").toLowerCase()){case "column_break":case "colb":g++;u[x].lines[A]={type:c.ColumnBreak,lines:[]};break;case "new_page":case "np":u[x].lines[A]=
{type:c.NewPage,lines:[]}}x=u;A=/\[(.+?)]/i;u=/\[(.+?)]/img;for(p in x)if(x[p].type==c.TextBlock||x[p].type==c.ChorusBlock)for(var v in x[p].lines)a=x[p].lines[v],"string"==typeof a&&(w=A.test(a),d=d||w,q=w&&1>ukeGeeks.toolsLite.trim(a.replace(u,"")).length,x[p].lines[v]={type:q?c.ChordOnlyText:w?c.ChordText:c.PlainText,lines:[a]},w&&""===b&&(w=a.match(A))&&(b=w[1]));p=x;k.body=n(p);1<g&&(k.body='<div class="'+e.ColumnWrap+" "+e.ColumnCount+g+'"><div class="'+e.Column+'">'+k.body+"</div></div>");
k.hasChords=d;v=l(p,c.Title);0<v.length&&(k.title=v[0]);v=l(p,c.Artist);0<v.length&&(k.artist=v[0]);v=l(p,c.Subtitle);0<v.length&&(k.st=v[0]);1<v.length&&(k.st2=v[1]);v=l(p,c.Album);0<v.length&&(k.album=v[0]);v=l(p,c.UkeGeeksMeta);0<v.length&&(k.ugsMeta=v);v=l(p,c.Key);0<v.length?k.key=v[0]:""!==b&&(k.key=b);v=l(p,c.ChordDefinition);if(0<v.length)for(var B in v)k.defs.push(ukeGeeks.chordImport.runLine("{define: "+v[B]+"}"));return k};var f={blocks:/\s*{\s*(start_of_tab|sot|start_of_chorus|soc|end_of_tab|eot|end_of_chorus|eoc)\s*}\s*/im,
...
ukeGeeks.tabs=function(){var a=ukeGeeks.settings.tabs,g=function(d,b,f){if(!f)return a.noteSpacing*d[0].length+b+a.dotRadius;f=d[0].length;var e=a.dotRadius;"|"==d[0][f-1]&&(--f,e=0);return a.noteSpacing*f+b+e};return{init:function(){},replace:function(d){d=d.getElementsByTagName("pre");for(var b in d)if("ugsTabs"==d[b].className){var f=d[b].innerHTML;d[b].innerHTML="";var e=d[b],f=f.split("\n"),c=[],n=void 0;for(n in f){var l=ukeGeeks.toolsLite.trim(f[n]);0<l.length&&c.push(l);if(4==c.length){var m=
c,c=e,k=void 0,l=void 0,m="string"==typeof m?m.split("\n"):m;if(!(4>m.length)){if(l="G"==m[3][0])for(var k=m,p=0;4>p;p++)k[p]=k[p].substr(1);for(var k=m,p=/([0-9]+)/g,r=[],h=0;4>h;h++)r[h]=k[h].match(p);for(var k=r,p=m,r=/([0-9]{2})/g,h=/([0-9])/g,t=[],q=0;4>q;q++)t[q]=p[q].replace(r,"-*"),t[q]=t[q].replace(h,"*");p=t;r=0;h=void 0;t=/-+$/gi;for(q=0;q<m.length;q++)h=m[q].trim().replace(t,""),h.length>r&&(r=h.length);h="";for(m=0;m<r;m++)h="|"==p[0][m]?h+"|":h+("*"==p[0][m]||"*"==p[1][m]||"*"==p[2][m]||
"*"==p[3][m]?"*":"-");m=/--/g;h=h.replace(m,"- ");m=/ -/g;for(t=h;;){h=h.replace(m,"  ");if(h==t)break;t=h}for(var m=p,p=h,h=[],t="",u=q=void 0,w=void 0,z=void 0,u=0;4>u;u++)h.push([]);for(u=0;4>u;u++)for(q=z=w=0;q<r;q++)" "!=p[q]&&("*"==m[u][q]?(t=k[u][z],z++):t="|"==p[q]?"|":"-",h[u][w]=t,w++);k=h;r=l?a.labelWidth:0;m=3*a.lineSpacing+2*a.dotRadius+a.bottomPadding;c="string"==typeof c?document.getElementById(c):c;c=ukeGeeks.canvasTools.addCanvas(c,g(k,r,!1),m);p=a.dotRadius+r;m=1+a.dotRadius;r=g(k,
r,!0);t=a.lineWidth/2;h=p+t;t=m+t;c.beginPath();for(q=0;4>q;q++)c.moveTo(h,t),c.lineTo(h+r,t),t+=a.lineSpacing;c.strokeStyle=a.lineColor;c.lineWidth=a.lineWidth;c.stroke();c.closePath();a:for(t in q=void 0,h={x:0,y:m},t=void 0,k){if(3<t)break a;h.x=p;for(var y in k[t]){q=k[t][y];if("|"==q){var x=parseInt(y,10),q=c,u=y==k[t].length-1?p+r:h.x,w=m,z=a,x=x+1<k[t].length-1&&"|"==k[t][x+1]||x==k[t].length-1&&"|"==k[t][x-1],A=z.lineWidth/2;q.beginPath();q.moveTo(u+A,w);q.lineTo(u+A,w+3*z.lineSpacing);q.strokeStyle=
z.lineColor;q.lineWidth=(x?4.5:1)*z.lineWidth;q.stroke();q.closePath()}else isNaN(q)||(ukeGeeks.canvasTools.drawDot(c,h,a.dotRadius,a.dotColor),ukeGeeks.canvasTools.drawText(c,{x:h.x,y:h.y+.5*a.dotRadius},q,a.textFont,a.textColor));h.x+=a.noteSpacing}h.y+=a.lineSpacing}if(l)for(l=ukeGeeks.settings.tuning.slice(0).reverse(),k=0;4>k;k++)ukeGeeks.canvasTools.drawText(c,{x:1,y:m+(k+.3)*a.lineSpacing},l[k],a.labelFont,a.lineColor,"left")}c=[]}}}}}};
